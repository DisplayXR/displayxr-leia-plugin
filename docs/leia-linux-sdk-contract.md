...
5. **X11 first.** X11/XCB was chosen over Wayland precisely because a client can query its own absolute screen position — which lenticular phase tracking needs. The DP factory receives the app's X11 `Window` when there is one and `NULL` otherwise; per-window phase does **not** ride the window handle on Linux (the weaver ignores the window's desktop position) but `srWeaverSetPresentOrigin` — see R-W7. The SDK's window-dependent features MUST degrade gracefully when no window is supplied; as shipped in 1.37.0.6048 the Vulkan weaver does **not** (§9 Q8).
...
Weaving into a sub-rectangle of the panel with correct lens phase is required for display-zones (mixed 2D/3D regions) and per-window weaving. This was the one capability the Windows SR SDK lacked (its canvas offsets are ignored — runtime#85; the Android CNSDK has `set_viewport` + `set_viewport_screen_position`, which is the shape to copy). **Honored on Linux:** LeiaSR#85 landed and `srWeaverSetPresentOrigin` ships in srSDK 1.0.0 — §8 R-W7.
...
| R-W3 optional window | ✅ | `.window = 0` = windowless/display-scoped in the API, documented "texture-in / weave-out", and **honoured by the shipped 1.37.0.6048 Vulkan weaver**: a weaver *constructed* with a null window takes the always-weave branch (`constructedWithoutWindow`, `WeaverBaseImpl.ipp:690-706`), confirmed on the 26.04 box (`window=0x0 = windowless/display-scoped`, weave path entered). The `Window handle is invalid in VulkanWeaver::setWindowHandle: weaving is disabled.` line fires only when a handle is cleared to 0 *after* construction — an earlier revision of this row misread it as a hard gate (2026-09-20); withdrawn, hygiene tracked on LeiaSR#248. X11 `Window` XID only — no `Display*`/xcb param (SDK opens its own connection). NB: even **with** a window, the Linux weaver does not track its desktop position — `Window2::getScreenRect` hardcodes rectX = rectY = 0 and takes only the size from the GL viewport, and the SR shared objects link zero X11 symbols — so window-scoped phase comes from `srWeaverSetPresentOrigin`, not from the window handle (see R-W7). |
...
| R-W7 viewport + phase origin | ✅ | `SetViewportVulkan`/`SetScissorRectVulkan` **plus `srWeaverSetPresentOrigin(SrWeaver, int32_t panelXPx, int32_t panelYPx)`** — the decoupled phase knob (LeiaSR#85) landed and ships in srSDK 1.0.0 as packaged in `leiasr-runtime 1.37.0.6048`: declared at `/opt/leiasr/include/sr/sr_weaver.h:386`, dispatch slot `pfnWeaverSetPresentOrigin` (`sr_loader.h:337`), implemented by `CVulkanWeaver::setPresentOrigin` in `/opt/leiasr/lib/libSimulatedRealityVulkan.so` (GL twin likewise). (`nm -D libLeiaSR_runtime.so` lists no exports — symbols are hidden and every call goes through the dispatch table; absence there is **not** evidence the entry point is missing.) Semantics per the header: panel-native pixels of the window client area's top-left relative to the panel's top-left; sticky across frames; effective origin = (panelXPx + viewportX, panelYPx + viewportY). **The plug-in already drives it:** DP slot `set_present_origin` (`leia_display_processor_linux.c:1496`), consumed in `process_atlas` (~:1263-1280), issued before every weave at `leia_sr_linux_sdk.c:1264` (logs once on failure). It matters in **any** monitor layout, not just multi-monitor: since the weaver never tracks window position (R-W3), a non-fullscreen window without it weaves at the wrong phase. |
...
4. **Phase origin API (R-W7)** — **answered: it shipped.** LeiaSR#85 landed: `srWeaverSetPresentOrigin(weaver, panelXPx, panelYPx)` is a first-class dispatch-table entry in srSDK 1.0.0 as packaged in `leiasr-runtime 1.37.0.6048`, and the plug-in drives it every weave (§8 R-W7). No longer an ask.
...
8. **Wayland** — **answered: weaves windowless.** `SrNativeWindowHandle` is an X11 `Window` (`unsigned long`) on Linux; the SDK opens its own X connection, and Wayland is unaddressed in 1.0.0 as an API surface. But a *native* Wayland session is fine: with no X11 window the plug-in passes `window = 0`, and a weaver *constructed* windowless always weaves (R-W3). An earlier revision of this item recorded 1.37.0.6048 logging `Window handle is invalid in VulkanWeaver::setWindowHandle: weaving is disabled.` and never weaving, and filed an ask on LeiaSR#224/#225 — that reading was wrong (the line fires only for `setWindowHandle(0)` after construction) and was withdrawn; the follow-up is LeiaSR#248 (make windowless an explicit mode). Phase comes from `srWeaverSetPresentOrigin` (R-W7), fed on Wayland by the GNOME geometry publisher (runtime#817). The remaining gap is validation, not API: the on-panel Wayland weave has not been phase-checked on real 3D hardware.
...
session start (vk_native compositor)
  └─ iface.create_dp_vk(vk_bundle, cmd_pool, x11_window (0 = none), VK_FORMAT_B8G8R8A8_UNORM, &dp)
