...
| R-W4 single tiled atlas | ◐ | ONE SBS `VkImageView` ✅; **no tile-grid param** (2×1 locked, no N-view) ❌; **no UV-flip toggle** ❌. Width/height ambiguity **settled by source** (`v2-vulkan-weaver` `vkweaver.cpp`): the values are stored and never read — the shader samples the view with computed UVs; pass full-SBS for documentation value. Input sampled at `SHADER_READ_ONLY_OPTIMAL` (descriptor write, verified in source). |
| R-W5 caller output target | ◐→✅ | Caller `VkFramebuffer` + w/h/format accepted, **or `framebuffer = 0` with a render pass already begun** — verified in source: fb=0 skips the weaver's own Begin/EndRenderPass entirely and just binds + draws. **Render-pass compatibility settled by source**: the weaver pipeline's pass is single color attachment @ outputFormat, `VK_SAMPLE_COUNT_1_BIT`, loadOp LOAD, `COLOR_ATTACHMENT_OPTIMAL` initial/final, no depth, one subpass — a matching caller pass is compatible per Vulkan §8.2. Output stays `COLOR_ATTACHMENT_OPTIMAL`. Residual ask: put this in SDK docs so it's contractual, not archaeology. |
