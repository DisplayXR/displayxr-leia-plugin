...
| R-W3 optional window | ◐ | `.window = 0` = windowless/display-scoped in the API, documented "texture-in / weave-out" — but **the shipped 1.37.0.6048 Vulkan weaver refuses it**: with a null window it logs `Window handle is invalid in VulkanWeaver::setWindowHandle: weaving is disabled.` and never weaves (observed 2026-09-20; the gate is Win32 heritage and fires before any phase input is read). X11 `Window` XID only — no `Display*`/xcb param (SDK opens its own connection). NB: even **with** a window, the Linux weaver does not track its desktop position — `Window2::getScreenRect` hardcodes rectX = rectY = 0 and takes only the size from the GL viewport, and the SR shared objects link zero X11 symbols — so window-scoped phase comes from `srWeaverSetPresentOrigin`, not from the window handle (see R-W7). |
...
8. **Wayland** — **effectively answered, and it does not weave:** `SrNativeWindowHandle` is an X11 `Window` (`unsigned long`) on Linux; the SDK opens its own X connection. Wayland is unaddressed in 1.0.0, and a *native* Wayland session is worse than degraded — with no X11 window the plug-in passes `window = 0`, and on 1.37.0.6048 the Vulkan weaver logs `Window handle is invalid in VulkanWeaver::setWindowHandle: weaving is disabled.` and never weaves (observed 2026-09-20). `srWeaverSetPresentOrigin` cannot rescue it: the null-window gate fires before any phase input is read. **New ask:** let the Linux weavers weave with no window once a present origin has been supplied — the gate is Win32 heritage (logged as a DisplayXR ask on LeiaSR#224/#225). Until then, Leia weaving on Linux is **X11/XWayland only**.
