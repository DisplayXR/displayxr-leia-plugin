...
> **Provenance:** written May 2026 against the runtime's Android POC branch
> (audit B15+B17+B18), salvaged here when leia vendor docs migrated to this
> repo. §2 (tile-to-eye) and §3 (UV flip) match the current `leia_cnsdk_weave`;
> §1's `leia_cnsdk_get_primary_face` is not yet ported to this plug-in — treat
> it as the spec for that port. Still pending: real Lume Pad hardware validation.
